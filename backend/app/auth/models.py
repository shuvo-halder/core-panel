...
class LoginRequest(BaseModel):
    username: str = Field(
        ..., min_length=1, max_length=128, description="Username or system identifier"
    )
...
class RoleRead(BaseModel):
    ...
    permissions: List[str] = Field(
        default_factory=list, description="List of permission identifiers e.g. users.read"
    )
...
class RoleCreate(BaseModel):
    ...
    permissions: List[str] = Field(
        default_factory=list, description="List of permission identifiers to grant"
    )
...
class UserCreate(BaseModel):
    ...
    roles: List[str] = Field(
        default_factory=list, description="Role names to assign, defaults to viewer if empty"
    )
    is_active: bool = True
